...
(async () => {
  const client = await createAuthenticatedClient({
    walletAddressUrl: "https://ilp.interledger-test.dev/ange", // Make sure the wallet address starts with https:// (not $), and has no trailing slashes
    privateKey: "private.key",
    keyId: "b794dc58-b3bb-42eb-81d5-e68ab5a023af",
  });

  const sendingWalletAddress = await client.walletAddress.get({
    url: "https://ilp.interledger-test.dev/agust", // Make sure the wallet address starts with https:// (not $)
  });
  const receivingWalletAddress = await client.walletAddress.get({
    url: "https://ilp.interledger-test.dev/rodri", // Make sure the wallet address starts with https:// (not $)
  });
